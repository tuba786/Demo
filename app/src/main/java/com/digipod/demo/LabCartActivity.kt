...
class LabCartActivity : AppCompatActivity() {
    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: LabCartAdapter
    ...
    private fun getCartData() {
        val labs = ArrayList<LabCartModel>()
        db.collection("labCarts").document(auth.currentUser!!.uid).collection("tests").get()
            .addOnSuccessListener { snaps ->
                for (document in snaps) {
                    val l = document.toObject(LabCartModel::class.java)
                    labs.add(l)
